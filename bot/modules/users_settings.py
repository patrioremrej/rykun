...
⏱ <b>Time Left:</b> <code>60 sec</code>""",
    ...
    "WATERMARK_ENABLED": (
        "Boolean",
        "Enable or disable watermark on videos.",
        "<i>Toggle to enable or disable watermark.</i> \n┖ <b>Time Left :</b> <code>60 sec</code>",
    ),
    "WATERMARK_TEXT": (
        "String",
        "Custom watermark text to overlay on videos.",
        "<i>Send the watermark text. Use dynamic variables if needed.</i> \n┖ <b>Time Left :</b> <code>60 sec</code>",
    ),
    "WATERMARK_COLOUR": (
        "String",
        "Watermark text color (e.g., white, red, #FFFFFF).",
        "<i>Send the watermark color.</i> \n┖ <b>Time Left :</b> <code>60 sec</code>",
    ),
    "WATERMARK_SIZE": (
        "Number",
        "Font size for the watermark text (e.g., 20).",
        "<i>Send the font size as a number.</i> \n┖ <b>Time Left :</b> <code>60 sec</code>",
    ),
    "WATERMARK_POSITION": (
        "String",
        "Position of the watermark (e.g., bottom-left, center, top-right).",
        "<i>Send the watermark position.</i> \n┖ <b>Time Left :</b> <code>60 sec</code>",
    ),
    "YT_DESP": (
        "String",
...
"""
        ...
        watermark_set = any(key.startswith("WATERMARK_") for key in user_dict)
        display_wm = "<b>Set</b>" if watermark_set else "<b>Not Set</b>"
...
┖ <b>Watermark</b> → {display_wm}"""

    elif stype == "watermark":
        if (
            user_dict.get("WATERMARK_ENABLED", False)
            or "WATERMARK_ENABLED" not in user_dict
            and getattr(Config, "WATERMARK_ENABLED", False)
        ):
            buttons.data_button(
                "Disable Watermark", f"userset {user_id} tog WATERMARK_ENABLED f"
            )
            wm_enabled = "Enabled"
        else:
            buttons.data_button(
                "Enable Watermark", f"userset {user_id} tog WATERMARK_ENABLED t"
            )
            wm_enabled = "Disabled"

        buttons.data_button("Text", f"userset {user_id} menu WATERMARK_TEXT")
        buttons.data_button("Colour", f"userset {user_id} menu WATERMARK_COLOUR")
        ...
        wm_text = user_dict.get("WATERMARK_TEXT", "<b>Not Set</b>")
        wm_colour = user_dict.get("WATERMARK_COLOUR", "<b>Not Set</b>")
        wm_size = user_dict.get("WATERMARK_SIZE", "<b>Not Set</b>")
        wm_position = user_dict.get("WATERMARK_POSITION", "<b>Not Set</b>")

        text = f"""⌬ <b>Watermark Settings :</b>
┟ <b>Name</b> → {user_name}
┃
┠ <b>Watermark</b> → <b>{wm_enabled}</b>
┠ <b>Text</b> → <code>{escape(str(wm_text))}</code>
┠ <b>Colour</b> → <code>{escape(str(wm_colour))}</code>
┠ <b>Size</b> → <code>{escape(str(wm_size))}</code>
┖ <b>Position</b> → <code>{escape(str(wm_position))}</code>
"""
...
async def set_option(_, message, option, rfunc):
    ...
    # elif option == "LEECH_DUMP_CHAT": # TODO: Add
        ...
        if value and isinstance(value, str):
            ...
            value = {}
    elif option == "WATERMARK_ENABLED":
        if value.lower() in ["true", "t", "1"]:
            value = True
        elif value.lower() in ["false", "f", "0"]:
            value = False
        else:
            await send_message(message, "Watermark enabled must be 'true' or 'false'.")
            return
    elif option == "WATERMARK_SIZE":
        if value.isdigit():
            value = int(value)
        else:
            await send_message(message, "Watermark size must be a number.")
            return
    # Add more validations if needed for WATERMARK_COLOUR or WATERMARK_POSITION
...
async def get_menu(option, message, user_id):
    ...
    if option in leech_options:
        ...
        back_to = "ffset"
        if option.startswith("WATERMARK_"):
            back_to = "watermark"
